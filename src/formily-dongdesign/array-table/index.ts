...
type JdTableProps = typeof JdTable
type JdColumnProps = typeof JdTableColumn
type JdPaginationProps = typeof JdPagination

const RecursionField = _RecursionField as unknown as Component

type ArrayTableProps = JdTableProps & {
  pagination?: JdPaginationProps | boolean
}

interface ObservableColumnSource {
  field: GeneralField
  fieldProps: IVoidFieldFactoryProps<any, any>
  columnProps: JdColumnProps & { title: string; asterisk: boolean }
  schema: Schema
...
type ColumnProps = JdColumnProps & {
  key: string | number
  asterisk: boolean
  render?: (
      ...
      return () => {
        ...
        const renderColumns = (startIndex?: Ref<number>) => {
          return columns.map(({ key, render, asterisk, ...props }) => {
            ...
            if (asterisk) {
              children.header = ({ column }: { column: JdColumnProps }) =>
                h('span', {}, [
                  h(
                    'span',
